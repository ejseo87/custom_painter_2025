...
class PomodorTimer extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    /* 
  String timeStringFormat(int seconds) {
    var duration = Duration(seconds: seconds.toInt());
    var timeString = duration.toString().split('.').first.substring(2, 7);
    //print('${timeString.split(':').first}  :   ${timeString.split(':')[1]}');
    return '${timeString.split(':').first}:${timeString.split(':')[1]}';
  }
...
class PomodorTimerScreen extends StatefulWidget {
  const PomodorTimerScreen({super.key});

  @override
  State<PomodorTimerScreen> createState() => _PomodorTimerScreenState();
}

class _PomodorTimerScreenState extends State<PomodorTimerScreen>
    with TickerProviderStateMixin {
  final _bgColor = Color(0xFF043953);
  final _inactvieColor = Color(0xFFEEEEEE);
  final _primaryColor = Color(0xFFFE676B);
